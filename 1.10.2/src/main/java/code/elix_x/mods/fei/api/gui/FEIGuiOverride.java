...
public class FEIGuiOverride implements IGuiElementsHandler<IGuiElement<FEIGuiOverride>> {
	...
	public void loadFromProfile(Profile profile){
		...
		for(JsonElement el : arr){
			...
			if(element != null){
				if(!(element instanceof INotDisableableFEIGuiElement))
					setEnabled(element, o.get("enabled").getAsBoolean());
				if(element instanceof ISaveableFEIGuiElement)
					((ISaveableFEIGuiElement) element).load(profile, o.get("data").getAsJsonObject());
			}
		}
	}
	...
	public void saveToProfile(Profile profile){
		...
		for(IGuiElement element : elements){
			...
			if(element instanceof ISaveableFEIGuiElement)
				o.add("data", ((ISaveableFEIGuiElement) element).save(profile));
			arr.add(o);
		}
	...
	public boolean handleKeyboardEvent(GuiScreen gui){
		if(focused != null && focused.handleKeyboardEvent(this, gui, Keyboard.getEventKeyState(), Keyboard.getEventKey(), Keyboard.getEventCharacter()))
			return true;
		for(IGuiElement<FEIGuiOverride> element : elements){
	...
	public boolean handleMouseEvent(GuiScreen gui){
		if(Mouse.getEventButton() > -1){
			...
			if(focused != null && focused.handleMouseEvent(this, gui, mouseX, mouseY, Mouse.getEventButtonState(), Mouse.getEventButton()))
				return true;
			for(IGuiElement<FEIGuiOverride> element : elements){
			...
			if(focused != null && focused.handleMouseEvent(this, gui, mouseX, mouseY, Mouse.getEventDWheel()))
				return true;
			for(IGuiElement<FEIGuiOverride> element : elements){
