	...
	public class FEIUtilsGridElement extends GridElement {
		...
		public void drawGuiPost(FEIGuiOverride fei, GuiScreen gui, int relX, int relY, int mouseX, int mouseY){
			if(this != currentSelected){
				...
				GuiButtonExt button = new GuiButtonExt(0, relX, relY, elementX, elementY, prop.getTexture() != null ? "" : prop.getText());
				button.enabled = prop.isEnabled();
				button.drawButton(gui.mc, mouseX, mouseY);
				if(prop.getTexture() != null){
					gui.mc.getTextureManager().bindTexture(prop.getTexture());
					drawTexturedRect(new Rectangle(relX + 2, relY + 2, elementX - 4, elementY - 4), new Rectangle(0, 0, 16, 16), 16, 16);
				}
			}
		}
		...
		public void drawGuiPostPost(FEIGuiOverride fei, GuiScreen gui, int relX, int relY, int mouseX, int mouseY) {
			if(this == currentSelected){
				if(backgroundColor.a > 0) gui.drawRect(relX - borderX, relY - borderY, relX + elementX + borderX, relY + getPropertiesCount() * (elementY + borderY), backgroundColor.argb());
				for(int i = 0; i < getPropertiesCount(); i++){
					IFEIUtilProperty prop = getAllProperties()[i];
					int y = relY + i * (elementY + borderY);
					GuiButtonExt button = new GuiButtonExt(0, relX, y, elementX, elementY, prop.getTexture() != null ? "" : prop.getText());
					button.enabled = prop.isEnabled();
					button.drawButton(gui.mc, mouseX, mouseY);
					if(prop.getTexture() != null){
						gui.mc.getTextureManager().bindTexture(prop.getTexture());
						drawTexturedRect(new Rectangle(relX + 2, y + 2, elementX - 4, elementY - 4), new Rectangle(0, 0, 16, 16), 16, 16);
					}
