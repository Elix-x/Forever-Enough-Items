...
public class JeiReflector implements IModPlugin, IGuiElement<FEIGuiOverride>, INotDisableableFEIGuiElement, ISaveableFEIGuiElement, IConfigurableFEIGuiElement {
	...
	private static final AField<ProxyCommonClient, GuiEventHandler> guiEventHandler = new AField(ProxyCommonClient.class, "guiEventHandler").setAccessible(true);
	private static final AField<GuiEventHandler, ItemListOverlay> itemListOverlayG = new AField(GuiEventHandler.class, "itemListOverlay").setAccessible(true);
	...
	public void onRuntimeAvailable(final IJeiRuntime jeiRuntime){
		new Thread(){
			...
			public void run(){
				...
				itemListOverlayG.set(guiEventHandler.get((ProxyCommonClient) JustEnoughItems.getProxy()), overlay);
