...
public class ProfileSettingsGuiScreen extends ElementalGuiScreen {
	...
	protected void addElements(){
		...
		String nameSetting = StatCollector.translateToLocal("fei.gui.profile.settings.name") + " ";
		...
		String iconSetting = StatCollector.translateToLocal("fei.gui.profile.settings.icon") + " ";
		...
		add(new StringGuiElement("Elements", xPos, nextY, 2, 2, StatCollector.translateToLocal("fei.gui.profile.settings.elements"), fontRendererObj, textColor));
		nextY += 2 + 8 + 2;
		...
		add(elementsList = new ListGuiElement("Elements List", xPos, nextY, guiWidth, 128, 20, 2, 2, new RGBA(0, 0, 0, 0)){
			...
			class GuiElementListElement extends ListElement {
				...
				public void drawGuiPost(IGuiElementsHandler handler, GuiScreen gui, int index, int x, int relY, int mouseX, int mouseY){
					GuiButtonExt button = new GuiButtonExt(0, x, relY, guiWidth - borderX - borderX, elementY, element instanceof IConfigurableFEIGuiElement ? StatCollector.translateToLocal(((IConfigurableFEIGuiElement) element).getUnlocalizedName()) : element.getName());
